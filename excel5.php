...
//database connection 
	$con = mysqli_connect("localhost","root","","adroit");
	if (!$con) {
...
//populate the data
	...
	while($data = mysqli_fetch_object($query)){
		...
		->setCellValue('F'.$row,$data->name2)
		->setCellValue('G'.$row,$data->email_id2)
        ->setCellValue('H'.$row,$data->number2)
        ->setCellValue('I'.$row,$data->name3)
		->setCellValue('J'.$row,$data->email_id3)
        ->setCellValue('K'.$row,$data->number3)
        ->setCellValue('L'.$row,$data->name4)
		->setCellValue('M'.$row,$data->email_id4)
        ->setCellValue('N'.$row,$data->number4)
        ->setCellValue('O'.$row,$data->name5)
		->setCellValue('P'.$row,$data->email_id5)
        ->setCellValue('Q'.$row,$data->number5);
	//increament
		$row++;
	}
//set column width
...
//make table header
	...
	->setCellValue('F3','Name 2')
	->setCellValue('G3','Email ID 2')
	->setCellValue('H3','Phone Number 2');
//merging the title
